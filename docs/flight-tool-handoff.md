...
> **마지막 배포 자산 버전: `v=20260707t`** (다섯 HTML 공통).
...
- **MRZ 둘째 줄·영문 라벨이 함께 안 보여도 일반 OCR 폴백** — MRZ 첫째 줄의 이름과
  여권면 전체의 여권번호·날짜를 병합한다. 카카오톡 화면 캡처처럼 여권이 작게 포함된
  이미지도 네 모서리 겹침 타일을 확대해 다시 읽고, 일부 필드만 잡혀도 수정 가능한 카드를 연다.
