...
const PARSE_RULES = {
    ...
    // 예: "9시15분~9시30분", "1시15~3시", "11시~11시30분", "6시30분부터", "11시~13시15분.."
    //   → 다음 줄과 병합 (부터/까지/쯤/경 꼬리표와 말줄임 ".." 도 허용)
    timeOnlyLine: /^\s*\d{1,2}\s*[시:]\s*(?:\d{1,2}\s*분?)?\s*(?:[~\-]\s*\d{1,2}\s*[시:]?\s*(?:\d{1,2}\s*분?)?)?(?:\s*(?:부터|까지|쯤|경))?\s*[.·…]*\s*$/,
    // 항목 맨 앞의 시간 표기 — 시간(윗줄)/일정(아랫줄) 두 줄로 표시하기 위해 분리
    // 예: "9시15분~9시30분 점심 도시락 구입" → time: "9시15분~9시30분", text: "점심 도시락 구입"
    //     "6시30분부터 본관 1층" → time: "6시30분부터", "11시~13시15분.. 산정 트레킹" → time: "11시~13시15분"
    leadTime: /^(\d{1,2}\s*[시:]\s*(?:\d{1,2}\s*분?)?\s*(?:[~\-]\s*\d{1,2}\s*[시:]?\s*(?:\d{1,2}\s*분?)?)?(?:\s*(?:부터|까지|쯤|경))?)\s*[.·…]*\s+(\S.*)$/,
    ...
    // 완결형 식사 명사 — '호텔 저녁식사(제공)'처럼 앞말이 장소 수식어면 쪼개지 말 것.
    // ('점심 Kappa Sushi'처럼 표기+메뉴가 분리되는 경우와 구분: 여긴 한 덩어리 식사)
    mealNoun: /^(아침식사|점심식사|저녁식사|조식|중식|석식)/,
    ...
    // 트레킹(본격 산행)이 식사보다 먼저 ('산정 트레킹&점심'은 트레킹이 본 활동),
    // 트레킹이 산책(가벼운 걷기)보다 먼저 ('야생화길 트레킹'은 트레킹)
    categoryKeywords: [
        ['move', ['공항', '도착', '출발', '이동', '버스', '기차', '페리', '체크인', '체크아웃', '픽업', '탑승', '항구']],
        ['spa',  ['온천', '찜질', '스파', '족욕', '노천탕', '온천욕']],
        // 트레킹/등산은 본 활동이므로 식사 표기보다 우선 ('산정 트레킹&점심' → 트레킹)
        ['trek', ['트레킹', '등산', '하이킹', '트레일', '종주', '산행']],
        // '한상'은 넣지 말 것: '16나한상'(十六羅漢岩, 바위 불상군)이 식사로 오분류됨 (검색으로 확인)
        // '아침'은 조식(아침 식사) — 대장 표기 습관
        ['food', ['점심', '저녁', '아침', '조식', '석식', '중식', '식사', '맛집', '스시', 'sushi', '뷔페', '밥집']],
        ['shop', ['쇼핑', '장보기', '마트', '시장', '아울렛', '기념품', '다이소', '약국', '베이커리', '편의점']],
...
function parseScheduleText(raw, opts = {}) {
    ...
    function addLine(line) {
        ...
        // 뒷부분이 완결형 식사 명사면(호텔 저녁식사) 앞말은 장소 수식어이므로 쪼개지 않음
        if (meal && !/^(점심|저녁|조식|석식)/.test(meal[1])
            && !PARSE_RULES.timeOnlyLine.test(meal[1])
            && !PARSE_RULES.mealNoun.test(meal[2])) {
            addItem(meal[1]);
            addMealItem(meal[2]);
            return;
        }
