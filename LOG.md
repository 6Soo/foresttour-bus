...
## 2026-07-31 — MRZ 없는 구형·메신저 캡처 여권 일반 OCR 폴백

- MRZ 둘째 줄이 잘렸거나 영문 라벨이 흐린 구형 여권도 첫째 줄의 느슨한 이름 패턴과
  여권면의 여권번호·날짜를 병합해 수정 가능한 탑승자 카드로 넘긴다.
- 카카오톡 화면 안에 여권 사진이 작게 들어간 경우 전체 화면 OCR이 채팅만 읽는 문제를
  해결하기 위해 네 모서리 68% 겹침 타일을 확대해 일반 OCR하고, block/sparse 판독 결과를 병합한다.
- 일부 필드만 읽혀도 여권 신호와 최소 식별정보가 있으면 실패 화면 대신 수정 화면을 연다.
  실패 안내도 MRZ 재촬영 강제가 아니라 이름·여권번호·생년월일이 크게 보이도록 안내한다.
- `tests/mrz-visual.test.js`에 MRZ 둘째 줄 누락·구형 여권 노이즈·항공 화면 오인 방지 회귀를 추가했다.
- 자산 버전은 다섯 HTML 공통 `v=20260707t`.
